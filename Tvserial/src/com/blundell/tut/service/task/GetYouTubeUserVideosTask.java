	...
	public void run() {
		...
		*/	// Load up the parser with the atom xml retrieved from youtube
		...
		//	List<Video> videos = parser.getVideos();
			...
			if(videos==null)
			{
				ArrayList<com.blundell.tut.parser.Video> videos1 =new ArrayList<com.blundell.tut.parser.Video>();
				Library lib = new Library(username, videos1);
			...
			{ArrayList<com.blundell.tut.parser.Video> videos =new ArrayList<com.blundell.tut.parser.Video>();
				Library lib = new Library(username, videos);
				// Pack the Library into the bundle to send back to the Activity
				Bundle data = new Bundle();
				data.putSerializable(LIBRARY, lib);
