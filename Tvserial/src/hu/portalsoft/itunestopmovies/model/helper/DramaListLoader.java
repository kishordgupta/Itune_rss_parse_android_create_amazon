 ...
 *
 */
public class DramaListLoader extends AsyncTaskLoader<List<ITunesMovie>> {

	/**
	 * Tag for logging
	 */
	private static final String TAG = DramaListLoader.class.getSimpleName();
	public static  String WebURL ="https://itunes.apple.com/us/rss/topmovies/limit=100/json";

	public DramaListLoader(Context context) {
		super(context);
	}
	
	public DramaListLoader(Context context,String Url) {
		super(context);
		WebURL=Url;
	}
	...
	public List<ITunesMovie> loadInBackground() {
        ...
        try {
        	...
        	//reset model
        	URLlist.Drama= new Model();
        	//reset last update timestamp
        	URLlist.Drama.setLastUpdateTimestamp(modelLastUpdateTimestamp);
        	//reset movies
        	URLlist.Drama.setMovies(modelMovies);
        	//
